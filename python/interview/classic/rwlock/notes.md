...
- **适用场景**：读多写少（配置缓存、路由表、文章浏览计数）。
- **复杂度**：读锁可并发；写锁独占；读者计数在 `guard` 保护下 O(1) 增减。
- **标准库对照**：C++17 `std::shared_mutex`；Java `ReentrantReadWriteLock`（读者优先）。
- **常见坑**：**写者饥饿**（读者源源不断）；升级锁（读锁内再申请写锁）易死锁，应用「先放读再拿写」。
